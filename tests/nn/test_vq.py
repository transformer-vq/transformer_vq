...
def test_get_shortcodes():
    ...
    vecs = codebook[-l:, :].reshape(1, 1, l, d)
    shortcodes, _ = get_shortcodes(vecs=vecs, codebook=codebook)
    np.testing.assert_allclose(
        actual=shortcodes,
        desired=jnp.arange(s - l, s).reshape([1, 1, l]),
    )
...
def test_get_codebook_ema_targets():
    ...
    vecs = c_sum[-l:, :].reshape(1, 1, l, d)
    vecs = vecs + 0.1 * jnp.ones_like(vecs)
    shortcodes, _ = get_shortcodes(vecs=vecs, codebook=c_sum)
    np.testing.assert_allclose(
        actual=shortcodes,
        desired=jnp.tile(jnp.arange(s - l, s).reshape([1, 1, l]), reps=[1, 1, 1]),
    )
...
def test_get_codebook_loss_ema(rng_fixture):
    ...
    c_gamma = 0.99
    c_sum = jax.nn.one_hot(jnp.arange(s), num_classes=d)
    c_count = 2 * jnp.ones([s], dtype=jnp.float64)
    params = dict(c_sum=c_sum, c_count=c_count)
    vecs = jax.random.normal(rng_fixture(0), shape=[1, 1, l, d], dtype=jnp.float64)
    shortcodes, _ = get_shortcodes(vecs=vecs, codebook=c_sum)

    # make expected updates
    r = jax.nn.one_hot(shortcodes, num_classes=s, dtype=jnp.float64)
    c_sum_hat = jnp.einsum("brcs,brck->sk", r, vecs)
    c_count_hat = jnp.einsum("brcs->s", r)
    c_sum_new_expected = c_gamma * c_sum + (1 - c_gamma) * c_sum_hat
    c_count_new_expected = c_gamma * c_count + (1 - c_gamma) * c_count_hat

    def loss_fn(params_, vecs_, shortcodes_, n_data_shard_, n_block_per_update_):
        return VectorQuantizer.get_codebook_loss(
            ...
            c_gamma=c_gamma,
            n_data_shard=jnp.array([n_data_shard_]),
...
def test_get_codebook_loss_ema_multidev(rng_fixture):
    ...
    l, s, d = 6, 5, 7
    c_gamma = 0.99
    c_sum = jax.random.normal(rng_fixture(0), shape=[s, d], dtype=jnp.float64)
    c_count = 2.0 * jnp.ones([s], dtype=jnp.float64)
    params = dict(c_sum=c_sum, c_count=c_count)
    vecs = jax.random.normal(rng_fixture(1), shape=[b, 1, l, d], dtype=jnp.float64)
    shortcodes, _ = get_shortcodes(vecs=vecs, codebook=c_sum)
    ...
    def grad_fn(params_, vecs_, shortcodes_, n_data_shard_, n_block_per_update_):
        def loss_fn(ps):
            return VectorQuantizer.get_codebook_loss(
                vecs=vecs_,
                shortcodes=shortcodes_,
    ...
    grads = grad_fn(
        ...
        n_data_shard_=jnp.full(fill_value=N_LOCAL_DEVICE, shape=[N_LOCAL_DEVICE, 1]),
...
def test_vector_quantizer_call(
    ...
    l = config.block_len
    d = config.d_k
    vecs = jax.random.normal(rng_fixture(1), shape=[b, 4, l, d], dtype=jnp.float64)
    rngs = dict(
        ephemeral=rng_fixture(2),
        timeless=rng_fixture(3),
    )
